...
export default async function handler(request, response) {
  const saveEditInput6RiskHandler = async (req, res) => {
  ...
  const saveEditInput6RiskHandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput6RiskHandler
  );

  await saveEditInput6RiskHandlerWithApiKeyValidation(request, response);
}
