...
export default async function handler(request, response) {
  const saveEditInput4OpHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputOp:", editInputOp);
  ...
  const saveEditInput4OpHandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput4OpHandler
  );

  await saveEditInput4OpHandlerWithApiKeyValidation(request, response);
}
