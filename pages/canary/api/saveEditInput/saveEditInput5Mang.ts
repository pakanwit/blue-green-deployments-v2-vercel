...
export default async function handler(request, response) {
  const saveEditInput5MangHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputMang:", editInputMang);
  ...
  const saveEditInput5MangHandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput5MangHandler
  );

  await saveEditInput5MangHandlerWithApiKeyValidation(request, response);
}
