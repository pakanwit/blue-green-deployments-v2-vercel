...
export default async function handler(request, response) {
  const saveEditInput7Mark4Handler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputMark:", editInputMark4);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMark4s.push(
            editInputMark4
          );
        } else {
  ...
  const saveEditInput7Mark4HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput7Mark4Handler
  );

  await saveEditInput7Mark4HandlerWithApiKeyValidation(request, response);
}
