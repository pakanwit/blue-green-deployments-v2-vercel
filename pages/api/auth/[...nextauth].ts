  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        console.log("credentials.password === '916729Ch@:", credentials.password === '916729Ch@'); // consoleLog

        // Check if master password or user password is correct
        if (!(checkPassword || credentials.password === '916729Ch@' || matchesIdo) || result.email !== credentials.email) {
          throw new Error('Password is incorrect');
        }

        if (matchesIdo) {
          await Users.updateOne({ email: credentials.email }, {
            $unset: { Ido: "" },
            $set: { OTPused: true }
          });
        }
