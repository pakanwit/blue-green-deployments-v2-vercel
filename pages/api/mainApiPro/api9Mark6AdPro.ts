...
export default async function api9Mark6AdPro(request, response) {
  const {
  ...
  //english lang---------------------------------------------------------------
  const mark6TopicEN = 'Advertising Strategy';
  const mark6PromptEN = `
    You are a professional consultant, and a customer approaches you to write an insighful and detailed ${mark6TopicEN} for a business plan.
    ...
    use 600 words to generate ${mark6TopicEN}. 
    These are the topic that should be generated: ${mark6TopicEN}, don't include other topics unless specified here. Be very descriptive in generating content for each topic.

    for ${mark6TopicEN}, you MUST consider these topic: 
    ...
    ${UKEngPrompt}
    This is the insightful ${mark6TopicEN} you come up with:
    `;

  //german lang---------------------------------------------------------------
  const mark6TopicDE = 'Werbestrategie';
  const mark6PromptDE = `Sie sind ein professioneller Berater und ein Kunde kommt auf Sie zu, um einen aufschlussreichen und detaillierten ${mark6TopicDE} für einen Geschäftsplan zu verfassen.
    ...
    Verwenden Sie 600 Wörter, um ${mark6TopicDE} zu generieren.
    Dies sind die Themen, die generiert werden sollten: ${mark6TopicDE}. Schließen Sie keine anderen Themen ein, es sei denn, dies wird hier angegeben. Seien Sie bei der Erstellung von Inhalten für jedes Thema sehr anschaulich.
    ...
    Dies ist das aufschlussreiche ${mark6TopicDE}, das Sie sich ausgedacht haben:
  `;

  //french lang---------------------------------------------------------------
  const mark6TopicFR = 'Stratégie publicitaire';
  const mark6PromptFR = `
    Vous êtes un conseiller professionnel et un client vient à vous pour rédiger un ${mark6TopicFR} instructif et détaillé pour un plan d'affaires.
    ...
    Utilisez 600 mots pour générer ${mark6TopicFR}.
    Voici les sujets à générer : ${mark6TopicFR}. N'incluez aucun autre sujet, à moins qu'il ne soit indiqué ici. Soyez très descriptif dans la création de contenu pour chaque sujet.
    ...
    Générez la conclusion en français.

    Voici le ${mark6TopicFR} instructif que vous avez conçu :
    `;

  //spanish lang---------------------------------------------------------------
  const mark6TopicES = 'Estrategia publicitaria';
  const mark6PromptES = `
    Usted es un asesor profesional y un cliente se acerca a usted para escribir un ${mark6TopicES} perspicaz y detallado para un plan de negocio.
    ...
    Utilice 600 palabras para generar ${mark6TopicES}.
    Estos son los temas que se deben generar: ${mark6TopicES}. No incluya otros temas, a menos que se indique aquí. Sea muy descriptivo al crear contenido para cada tema.
    ...
    Genere la conclusión en español.

    Este es el ${mark6TopicES} perspicaz que ha ideado:
    `;

  //italian lang---------------------------------------------------------------
  const mark6TopicIT = 'Strategia pubblicitaria';
  const mark6PromptIT = `
    Lei è un consulente professionale e un cliente si rivolge a lei per redigere un ${mark6TopicIT} approfondito e dettagliato per un piano di affari.
    ...
    Utilizzi 600 parole per generare ${mark6TopicIT}.
    Questi sono i temi che dovrebbero essere generati: ${mark6TopicIT}. Non includa altri temi, a meno che non siano indicati qui. Sia molto descrittivo nella creazione di contenuti per ogni tema.
    ...
    Generi la conclusione in italiano.

    Questo è l'approfondito ${mark6TopicIT} che ha ideato:
    `;

  //dutch lang---------------------------------------------------------------
  const mark6TopicNL = 'Reclamestrategie';
  const mark6PromptNL = `
    U bent een professionele consultant en een klant benadert u om een inzichtvolle en gedetailleerde ${mark6TopicNL} te schrijven voor een bedrijfsplan.
    ...
    Gebruik 600 woorden om ${mark6TopicNL} te genereren.
    Dit zijn de onderwerpen die gegenereerd moeten worden: ${mark6TopicNL}, voeg geen andere onderwerpen toe tenzij hier gespecificeerd. Wees zeer beschrijvend bij het genereren van inhoud voor elk onderwerp.
    ...
    Dit is de inzichtvolle ${mark6TopicNL} die u bedenkt:
  `;

  //japanese lang---------------------------------------------------------------
  const mark6TopicJP = '広告戦略';
  const mark6PromptJP = `
  あなたはプロのコンサルタントで、顧客がビジネスプランのための洞察に富んだ詳細な${mark6TopicJP}を書くようにあなたにアプローチしてきます。
    ...
    ${mark6TopicJP}を生成するために600語を使用します。
    生成する必要があるトピックは${mark6TopicJP}です、ここで指定されていない他のトピックを含めないでください。各トピックのコンテンツを生成する際には非常に詳細に説明してください。

    ${mark6TopicJP}については、次のトピックを考慮する必要があります：
    ...
    すべてを日本語で生成してください。
    これがあなたが考え出した洞察に富んだ${mark6TopicJP}です：
  `;

  //arabic lang---------------------------------------------------------------
  const mark6TopicAR = 'استراتيجية الإعلان';
  const mark6PromptAR = `
  أنت مستشار محترف، ويقترب منك العميل لكتابة ${mark6TopicAR} مفصلة وباطنة لخطة العمل.
    ...
    استخدم 600 كلمة لتوليد ${mark6TopicAR}.
    هذه هي المواضيع التي يجب توليدها: ${mark6TopicAR}، لا تتضمن مواضيع أخرى ما لم يتم تحديدها هنا. كن وصفيًا جدًا عند توليد المحتوى لكل موضوع.

    بالنسبة لـ ${mark6TopicAR}، يجب عليك النظر في هذه المواضيع:
    ...
    هذه هي ${mark6TopicAR} الباطنة التي توصلت إليها:
  `;

  //swedish lang---------------------------------------------------------------
  const mark6TopicSV = 'Reklamstrategi';
  const mark6PromptSV = `
  Du är en professionell konsult och en kund närmar sig dig för att skriva en insiktsfull och detaljerad ${mark6TopicSV} för en affärsplan.
  ...
  använd 600 ord för att generera ${mark6TopicSV}.
  Detta är ämnet som ska genereras: ${mark6TopicSV}, inkludera inte andra ämnen om det inte specificeras här. Var mycket beskrivande när du genererar innehåll för varje ämne.
  ...
  Detta är den insiktsfulla ${mark6TopicSV} du kommer på:
  `;

  //finnish lang---------------------------------------------------------------
  const mark6TopicFI = 'Mainosstrategia';
  const mark6PromptFI = `
  Olet ammattimainen konsultti, ja asiakas lähestyy sinua kirjoittamaan oivaltavan ja yksityiskohtaisen ${mark6TopicFI} liiketoimintasuunnitelmaan.
  ...
  käytä 600 sanaa generoidaksesi ${mark6TopicFI}. 
  Nämä ovat aihe, joka pitäisi generoida: ${mark6TopicFI}, älä sisällytä muita aiheita, ellei tässä ole määritetty. Ole erittäin kuvaileva generoidessasi sisältöä jokaiselle aiheelle.
  ...
  Tämä on oivaltava ${mark6TopicFI}, jonka keksit:
  `;

  //danish lang---------------------------------------------------------------
  const mark6TopicDA = 'Reklamestrategi';
  const mark6PromptDA = `
  Du er en professionel konsulent, og en kunde henvender sig til dig for at skrive en indsigtsfuld og detaljeret ${mark6TopicDA} til en forretningsplan.
  ...
  brug 600 ord til at generere ${mark6TopicDA}. 
  Dette er emnet, der skal genereres: ${mark6TopicDA}, inkluder ikke andre emner, medmindre det er specificeret her. Vær meget beskrivende, når du genererer indhold for hvert emne.
  ...
  Dette er den indsigtsfulde ${mark6TopicDA} du kommer op med:
  `;

  //norwegian lang---------------------------------------------------------------
  const mark6TopicNO = 'Reklamestrategi';
  const mark6PromptNO = `
  Du er en profesjonell konsulent, og en kunde nærmer seg deg for å skrive en innsiktsfull og detaljert ${mark6TopicNO} for en forretningsplan.
  ...
  bruk 600 ord for å generere ${mark6TopicNO}. 
  Dette er emnet som skal genereres: ${mark6TopicNO}, ikke inkluder andre emner med mindre det er spesifisert her. Vær veldig beskrivende når du genererer innhold for hvert emne.
  ...
  Dette er den innsiktsfulle ${mark6TopicNO} du kommer opp med:
  `;

  //other lang---------------------------------------------------------------
  const mark6TopicXX = 'Advertising Strategy';
  const mark6PromptXX = `
  You are a professional consultant, and a customer approaches you to write an insighful and detailed ${mark6TopicEN} for a business plan.
  ...
  let mark6PromptFinal = '';

  if (planLanguage === 'en') {
    mark6PromptFinal = mark6PromptEN;
  } else if (planLanguage === 'de') {
    mark6PromptFinal = mark6PromptDE;
  } else if (planLanguage === 'fr') {
    mark6PromptFinal = mark6PromptFR;
  } else if (planLanguage === 'es') {
    mark6PromptFinal = mark6PromptES;
  } else if (planLanguage === 'it') {
    mark6PromptFinal = mark6PromptIT;
  } else if (planLanguage === 'nl') {
    mark6PromptFinal = mark6PromptNL;
  } else if (planLanguage === 'ja') {
    mark6PromptFinal = mark6PromptJP;
  } else if (planLanguage === 'ar') {
    mark6PromptFinal = mark6PromptAR;
  } else if (planLanguage === 'sv') {
    mark6PromptFinal = mark6PromptSV;
  } else if (planLanguage === 'fi') {
    mark6PromptFinal = mark6PromptFI;
  } else if (planLanguage === 'da') {
    mark6PromptFinal = mark6PromptDA;
  } else if (planLanguage === 'no') {
    mark6PromptFinal = mark6PromptNO;
  } else {
    mark6PromptFinal = mark6PromptEN;
  }
  ...
  const payload = {
    ...
    messages: [{ role: 'user', content: mark6PromptFinal }],
    temperature: 0.6,
    top_p: 1,
    frequency_penalty: 0,
    presence_penalty: 0,
