...
export default async function api4Mark1ObjPro(request, response) {
  const {
  ...
  const mark1PromptDE = `Sie sind ein professioneller Berater und ein Kunde kommt auf Sie zu, um ${mark1TopicDE} für einen Geschäftsplan zu schreiben.
  ...
  Beginnen Sie die Vervollständigung mit „<h3>Marketing</h3>“, gefolgt von „<h4>${mark1TopicDE}</h4>“. Es sollte nur ein <h3>-Tag für <h3>Marketing</h3> und ein <h4>-Tag für <h4>${mark1TopicDE}</h4> vorhanden sein. Es müssen 3 <h5>-Tags vorhanden sein, nämlich <h5>Kurzfristige Ziele<h5>, <h5>Mittelfristige Ziele<h5> und <h5>Langfristige Ziele<h5>.
  
  Fertigstellung auf Deutsch generieren.
  Dies ist das ${mark1TopicDE}, das Sie sich ausgedacht haben:`;

  //french lang --------------------------------------------------------
  const mark1TopicFR = 'objectifs commerciaux';
  const mark1PromptFR = `
    Vous êtes un consultant professionnel et un client vous approche pour écrire ${mark1TopicFR} pour un plan d'affaires.
  ...
  Commencez par "<h3>Marketing</h3>" suivi de "<h4>${mark1TopicFR}</h4>". Il ne doit y avoir qu'une seule balise <h3> qui est <h3>Marketing</h3> et une seule balise <h4> qui est <h4>${mark1TopicFR}</h4>. Il doit y avoir 3 balises <h5> qui sont <h5>Objectifs à court terme<h5>, <h5>Objectifs à moyen terme<h5> et <h5>Objectifs à long terme<h5>.

  Voici le ${mark1TopicFR} que vous avez élaboré :
    `;

  //spanish lang --------------------------------------------------------
  const mark1TopicES = 'objetivos comerciales';
  const mark1PromptES = `
    Usted es un consultor profesional y un cliente se acerca para que escriba ${mark1TopicES} para un plan de negocios.
  ...
  El ${mark1TopicES} DEBE INCLUIR ESTOS TEMAS: objetivos a corto plazo (1-2 años), objetivos a medio plazo (3-5 años) y objetivos a largo plazo (más de 5 años). NO incluir otros temas a menos que se especifique aquí. Cada tema tendrá múltiples objetivos, cada uno dentro de una etiqueta <li>. Los objetivos a corto plazo deben ser modestos y los de largo plazo ambiciosos.
  ...
  Comience con "<h3>Marketing</h3>" seguido de "<h4>${mark1TopicES}</h4>". Solo debe haber una etiqueta <h3> que es <h3>Marketing</h3> y una etiqueta <h4> que es <h4>${mark1TopicES}</h4>. Debe haber 3 etiquetas <h5> que son <h5>Objetivos a Corto Plazo<h5>, <h5>Objetivos a Medio Plazo<h5> y <h5>Objetivos a Largo Plazo<h5>.

  Este es el ${mark1TopicES} que ha desarrollado:
    `;

  //italian lang --------------------------------------------------------
  const mark1TopicIT = 'obiettivi aziendali';
  const mark1PromptIT = `
    Sei un consulente professionale e un cliente ti avvicina per scrivere ${mark1TopicIT} per un piano aziendale.
  ...
  Inizia con "<h3>Marketing</h3>" seguito da "<h4>${mark1TopicIT}</h4>". Dovrebbe esserci un solo tag <h3> che è <h3>Marketing</h3> e un tag <h4> che è <h4>${mark1TopicIT}</h4>. Ci devono essere 3 tag <h5> che sono <h5>Obiettivi a Breve Termine<h5>, <h5>Obiettivi a Medio Termine<h5> e <h5>Obiettivi a Lungo Termine<h5>.

  Questo è il ${mark1TopicIT} che hai elaborato:
    `;

  // dutch lang --------------------------------------------------------
  const mark1TopicNL = 'bedrijfsdoelstellingen';
  const mark1PromptNL = `
    U bent een professionele consultant en een klant benadert u om ${mark1TopicNL} te schrijven voor een bedrijfsplan.
    ...
    Begin de voltooiing met "<h3>Marketing</h3>" gevolgd door "<h4>${mark1TopicNL}</h4>". Er mag maar één <h3> tag zijn, namelijk <h3>Marketing</h3> en één <h4> tag, namelijk <h4>${mark1TopicNL}</h4>. Er moeten 3 <h5> tags zijn, namelijk <h5>Korte termijn doelstellingen<h5>, <h5>Middellange termijn doelstellingen<h5> en <h5>Lange termijn doelstellingen<h5>.
    Genereer alles in het Nederlands.
    Dit is de ${mark1TopicNL} die u heeft bedacht:
    `;

  // japanese lang --------------------------------------------------------
  const mark1TopicJP = 'ビジネス目標';
  const mark1PromptJP = `
    あなたはプロのコンサルタントで、顧客がビジネスプランのための${mark1TopicJP}を書くようにあなたにアプローチしてきました。
    ...
    これらはさらなる指示です：
    ビジネスの詳細を繰り返さないでください。
    ${mark1TopicJP}は、これらのトピックを含める必要があります：短期目標（1-2年）、中期目標（3-5年）、長期目標（5年以上）。ここで指定されていない他のトピックを含めないでください。各トピックは、複数の目標を<li>タグで囲んで持つでしょう。短期目標は控えめでなければならず、長期目標は野心的でなければなりません。
    ...
    応答をhtmlで生成します。
    完成を"<h3>マーケティング</h3>"で始め、その後に"<h4>${mark1TopicJP}</h4>"を続けます。"<h3>マーケティング</h3>"という1つの<h3>タグだけがあり、"<h4>${mark1TopicJP}</h4>"という1つの<h4>タグだけがあるべきです。"<h5>短期目標<h5>"、"<h5>中期目標<h5>"、"<h5>長期目標<h5>"という3つの<h5>タグがあるべきです。
    すべてを日本語で生成します。
    これがあなたが考え出した${mark1TopicJP}です：
    `;

  //arabic lang --------------------------------------------------------
  const mark1TopicAR = 'أهداف العمل';
  const mark1PromptAR = `
    أنت مستشار محترف، وقد اقترب منك العميل لكتابة ${mark1TopicAR} لخطة العمل.
  ...
  يجب أن يتضمن ${mark1TopicAR} هذه المواضيع: الأهداف القصيرة الأجل (1-2 سنة)، الأهداف المتوسطة الأجل (3-5 سنوات) الأهداف الطويلة الأجل (أكثر من 5 سنوات). لا تتضمن مواضيع أخرى ما لم يتم تحديدها هنا. سيكون لكل موضوع أهداف متعددة محاطة بوسم <li>. يجب أن تكون الأهداف القصيرة الأجل متواضعة والأهداف الطويلة الأجل طموحة.
  ...
  ابدأ الإكمال بـ "<h3>التسويق</h3>" تليها "<h4>${mark1TopicAR}</h4>". يجب أن يكون هناك وسم <h3> واحد فقط وهو <h3>التسويق</h3> ووسم <h4> واحد فقط وهو <h4>${mark1TopicAR}</h4>. يجب أن يكون هناك 3 أوسمة <h5> وهي <h5>الأهداف القصيرة الأجل<h5>، <h5>الأهداف المتوسطة الأجل<h5> و<h5>الأهداف الطويلة الأجل<h5>.
  أنشئ كل شيء باللغة العربية.
  هذه هي ${mark1TopicAR} التي ابتكرتها:
    `;
  ...
  const mark1PromptSV = `
    ...
    ${mark1TopicSV} MÅSTE INKLUDERA DESSA ÄMNEN: kortsiktiga mål (1-2 år), medellånga mål (3-5 år) långsiktiga mål (mer än 5 år). INKLUDERA INTE andra ämnen om det inte specificeras här. Varje ämne kommer att ha flera mål inramade i <li> taggen. Kortsiktiga mål bör vara modesta och långsiktiga mål bör vara ambitiösa.
    ...
    Börja slutförandet med "<h3>Marknadsföring</h3>" följt av "<h4>${mark1TopicSV}</h4>". Det bör bara finnas en <h3> tagg som är <h3>Marknadsföring</h3> och en <h4> tagg som är <h4>${mark1TopicSV}</h4>. Det måste finnas 3 <h5> taggar som är <h5>Kortsiktiga mål<h5>, <h5>Medellånga mål<h5> och <h5>Långsiktiga mål<h5>.
    Generera allt på svenska.
    Detta är ${mark1TopicSV} du kom upp med:
    `;
  ...
  Aloita täydennys "<h3>Markkinointi</h3>" seurattuna "<h4>${mark1TopicFI}</h4>". Pitäisi olla vain yksi <h3> -tagi, joka on <h3>Markkinointi</h3> ja yksi <h4> -tagi, joka on <h4>${mark1TopicFI}</h4>. On oltava 3 <h5> -tagia, jotka ovat <h5>Lyhyen aikavälin tavoitteet<h5>, <h5>Keskipitkän aikavälin tavoitteet<h5> ja <h5>Pitkän aikavälin tavoitteet<h5>. 
  ...
  Generer svar i html.
  Begynd udfyldelsen med "<h3>Marketing</h3>" efterfulgt af "<h4>${mark1TopicDA}</h4>". Der skal kun være en <h3> tag som er <h3>Marketing</h3> og en <h4> tag som er <h4>${mark1TopicDA}</h4>. Der skal være 3 <h5> tags som er <h5>Kortfristede Mål<h5>, <h5>Mellemlange Mål<h5> og <h5>Lange Mål<h5>. 
  Generer alt på dansk.
  Dette er ${mark1TopicDA} du kom op med:
    `;

  // norwegian lang --------------------------------------------------------
  const mark1TopicNO = 'bedriftsmål';
  const mark1PromptNO = `
    Du er en profesjonell konsulent, og en kunde nærmer seg deg for å skrive ${mark1TopicNO} for en forretningsplan.
  ...
  Begynn utfyllingen med "<h3>Markedsføring</h3>" etterfulgt av "<h4>${mark1TopicNO}</h4>". Det skal bare være en <h3> tag som er <h3>Markedsføring</h3> og en <h4> tag som er <h4>${mark1TopicNO}</h4>. Det må være 3 <h5> tags som er <h5>Kortsiktige Mål<h5>, <h5>Mellomlangsiktige Mål<h5> og <h5>Lange Mål<h5>. 
  Generer alt på norsk.
  Dette er ${mark1TopicNO} du kom opp med:
    `;

  // other lang --------------------------------------------------------
  const mark1TopicOther = 'business objectives';
  const mark1PromptOther = `
    You are a professional consultant, and a customer approaches you to write ${mark1TopicEN} for a business plan.
  ...
  if (planLanguage === 'en') {
    ...
    mark1PromptFinal = mark1PromptJP;
