...
export default async function api7Mark4ProductPro(request, response) {
  const {
  ...
  //english lang-----------------------------------------------------------------------
  const mark4TopicEN = 'Product Strategy';
  const mark4PromptEN = `
    ...
    ${productInfoPrompt}
    These products or services details MUST be included in the ${mark4TopicEN}.
    
    use 700 words to generate ${mark4TopicEN}. 

    for both ${mark4TopicEN}, you MUST consider this targeting and postioning data:
    ...
    Generate response in html surrounding ${mark4TopicEN} topics with h5 tag. 
    ...
    ${UKEngPrompt}
    This is the long, detailed, and lengthy ${mark4TopicEN} you came up with:
    `;

  //german lang---------------------------------------------------------------------
  const mark4TopicDE = 'Produktstrategie';
  const mark4PromptDE = `Sie sind ein professioneller Berater und ein Kunde kommt zu Ihnen, um einen langen und detaillierten ${mark4TopicDE} für einen Geschäftsplan zu schreiben.
    ...
    ${productInfoPrompt}
    Diese Produkt- oder Dienstleistungsdetails MÜSSEN in den ${mark4TopicDE} aufgenommen werden.
    
    Verwenden Sie 700 Wörter, um ${mark4TopicDE} zu erzeugen. 
    ...
    Erzeugen Sie eine Antwort in html, die die ${mark4TopicDE}-Themen mit dem h5-Tag umgibt. 
    ...
    Dies ist das lange, detaillierte und ausführliche ${mark4TopicDE}, das du dir ausgedacht hast:
  `;
  //french lang---------------------------------------------------------------------
  const mark4TopicFR = 'Stratégie de produit';
  const mark4PromptFR = `
    Vous êtes un conseiller professionnel et un client vient à vous pour rédiger un ${mark4TopicFR} long et détaillé pour un plan d'affaires.
  ...
  ${productInfoPrompt}
  Ces détails de produit ou service DOIVENT être inclus dans le ${mark4TopicFR}.

  Utilisez 700 mots pour générer ${mark4TopicFR}.
  ...
  Générez une réponse en html qui entoure les sujets de ${mark4TopicFR} avec la balise h5.
  ...
  Voici le ${mark4TopicFR} long, détaillé et approfondi que vous avez conçu :
    `;

  //spanish lang---------------------------------------------------------------------
  const mark4TopicES = 'Estrategia de producto';
  const mark4PromptES = `
    Usted es un consultor profesional y un cliente se acerca a usted para escribir un ${mark4TopicES} largo y detallado para un plan de negocios.
  ...
  ${productInfoPrompt}
  Estos detalles del producto o servicio DEBEN incluirse en el ${mark4TopicES}.

  Use 700 palabras para generar ${mark4TopicES}.
  ...
  Genere una respuesta en html que envuelva los temas de ${mark4TopicES} con la etiqueta h5.
  ...
  Este es el ${mark4TopicES} largo, detallado y minucioso que ha diseñado:
    `;

  //italian lang---------------------------------------------------------------------
  const mark4TopicIT = 'Strategia del prodotto';
  const mark4PromptIT = `
    Lei è un consulente professionale e un cliente si rivolge a lei per scrivere un ${mark4TopicIT} lungo e dettagliato per un piano di business.
    ...
    ${productInfoPrompt}
    Questi dettagli del prodotto o servizio DEVONO essere inclusi nel ${mark4TopicIT}.

    Utilizzi 700 parole per generare ${mark4TopicIT}.
    ...
    Generi una risposta in html che avvolga i temi di ${mark4TopicIT} con il tag h5.
    ...
    Questo è il ${mark4TopicIT} lungo, dettagliato e approfondito che ha ideato:
    `;

  //dutch lang---------------------------------------------------------------------
  const mark4TopicNL = 'Productstrategie';
  const mark4PromptNL = `
    U bent een professionele consultant en een klant benadert u om een lange en gedetailleerde ${mark4TopicNL} te schrijven voor een bedrijfsplan.
    ...
    ${productInfoPrompt}
    Deze product- of dienstdetails MOETEN worden opgenomen in de ${mark4TopicNL}.

    Gebruik 700 woorden om ${mark4TopicNL} te genereren.
    ...
    Genereer een reactie in html die de onderwerpen van ${mark4TopicNL} omringt met de h5-tag.
    ...
    Dit is de lange, gedetailleerde en uitgebreide ${mark4TopicNL} die u heeft bedacht:
    `;

  //japanese lang---------------------------------------------------------------------
  const mark4TopicJP = '製品戦略';
  const mark4PromptJP = `
    あなたはプロのコンサルタントであり、顧客がビジネスプランのための詳細で長い${mark4TopicJP}を書くように依頼してきました。
    ...
    ${productInfoPrompt}
    これらの製品またはサービスの詳細は${mark4TopicJP}に必ず含める必要があります。

    ${mark4TopicJP}を生成するために700語を使用します。
    ...
    ${mark4TopicJP}のトピックをh5タグで囲んだhtmlのレスポンスを生成します。
    完成を"<h4>4P</h4>"に続いて"<h5>製品戦略</h5>"で始め、これらのトピック"<h4>4P</h4>"と"<h5>製品戦略</h5>"を完成の中で再度繰り返さないでください。
    完成の中で" "を使用しないでください。
    すべてを日本語で生成します。
    これがあなたが考え出した長く、詳細で、長い${mark4TopicJP}です：
    `;

  //arabic lang---------------------------------------------------------------------
  const mark4TopicAR = 'استراتيجية المنتج';
  const mark4PromptAR = `
    ...
    ${productInfoPrompt}
    يجب تضمين تفاصيل هذه المنتجات أو الخدمات في ${mark4TopicAR}.

    استخدم 700 كلمة لتوليد ${mark4TopicAR}.
    ...
    قم بتوليد الرد باللغة العربية محيطًا بمواضيع ${mark4TopicAR} بوسم h5.
    ...
    هذه هي ${mark4TopicAR} الطويلة والمفصلة والطويلة التي ابتكرتها:
    `;

  // swedish lang---------------------------------------------------------------------
  const mark4TopicSV = 'Produktstrategi';
  const mark4PromptSV = `
    ...
    ${productInfoPrompt}
    Dessa produkter eller tjänster detaljer MÅSTE ingå i ${mark4TopicSV}.

    använd 700 ord för att generera ${mark4TopicSV}.
    ...
    Generera svar i html som omger ${mark4TopicSV} ämnen med h5 tagg.
    ...
    Detta är den långa, detaljerade och långa ${mark4TopicSV} du kom på:
    `;

  //finnish lang---------------------------------------------------------------------
  const mark4TopicFI = 'Tuotekehitys';
  const mark4PromptFI = `
    ...
    ${productInfoPrompt}
    Nämä tuotteiden tai palveluiden tiedot ON sisällytettävä ${mark4TopicFI}.

    käytä 700 sanaa generoimaan ${mark4TopicFI}.
    ...
    Generoi vastaus html:ssä ympäröimällä ${mark4TopicFI} aiheet h5-tagilla.
  ...
  //danish lang---------------------------------------------------------------------
  const mark4TopicDA = 'Produktstrategi';
  const mark4PromptDA = `
    Du er en professionel konsulent, og en kunde nærmer dig for at skrive en lang og detaljeret ${mark4TopicDA} til en forretningsplan.
    ...
    ${productInfoPrompt}
    Disse produkter eller tjenester detaljer SKAL inkluderes i ${mark4TopicDA}.

    brug 700 ord til at generere ${mark4TopicDA}.
    ...
    Generer svar i html omkring ${mark4TopicDA} emner med h5 tag.
  ...
  //norwegian lang---------------------------------------------------------------------
  const mark4TopicNO = 'Produktstrategi';
  const mark4PromptNO = `
    Du er en profesjonell konsulent, og en kunde nærmer seg deg for å skrive en lang og detaljert ${mark4TopicNO} for en forretningsplan.
    ...
    ${productInfoPrompt}
    Disse produktene eller tjenestene detaljer MÅ inkluderes i ${mark4TopicNO}.

    bruk 700 ord for å generere ${mark4TopicNO}.
    ...
    Generer svar i html rundt ${mark4TopicNO} emner med h5 tag.
    ...
    `;
  //other lang---------------------------------------------------------------------
  const mark4TopicOther = 'Product Strategy';
  const mark4PromptOther = `
    You are a professional consultant, and a customer approaches you to write a long and detailed ${mark4TopicEN} for a business plan.
    ...
    For product strategy, describe the product or service in detail. This is Important: product strategy MUST align itself to the positioning data. The best product strategy aligns with positioning. Product Strategy topic should include: product description, product differentiation, product development, and product branding. These topics should be wrapped in <h6> tags. Each topic should describe each product or service in detail. 
  ...
  if (planLanguage === 'en') {
    ...
    mark4PromptFinal = mark4PromptJP;
