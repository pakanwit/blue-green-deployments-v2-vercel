...
export default async function handler(request, response) {
  const saveEditInput11Op2Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputOp:', editInputOp2);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputOp2s.push(
            editInputOp2,
          );
        } else {
  ...
  const saveEditInput11Op2HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput11Op2Handler,
  );

  await saveEditInput11Op2HandlerWithApiKeyValidation(request, response);
}
