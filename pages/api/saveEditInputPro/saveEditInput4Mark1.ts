...
export default async function handler(request, response) {
  const saveEditInput4Mark1Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputMark:', editInputMark1);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMark1s.push(
            editInputMark1,
          );
        } else {
  ...
  const saveEditInput4Mark1HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput4Mark1Handler,
  );

  await saveEditInput4Mark1HandlerWithApiKeyValidation(request, response);
}
