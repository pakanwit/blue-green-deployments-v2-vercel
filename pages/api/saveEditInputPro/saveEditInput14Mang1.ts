...
export default async function handler(request, response) {
  const saveEditInput14Mang1Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputMang:', editInputMang1);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMang1s.push(
            editInputMang1,
          );
        } else {
  ...
  const saveEditInput14Mang1HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput14Mang1Handler,
  );

  await saveEditInput14Mang1HandlerWithApiKeyValidation(request, response);
}
