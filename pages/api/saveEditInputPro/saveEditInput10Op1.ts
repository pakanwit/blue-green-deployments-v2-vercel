...
export default async function handler(request, response) {
  const saveEditInput10Op1Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputOp:', editInputOp1);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputOp1s.push(
            editInputOp1,
          );
        } else {
  ...
  const saveEditInput10Op1HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput10Op1Handler,
  );

  await saveEditInput10Op1HandlerWithApiKeyValidation(request, response);
}
