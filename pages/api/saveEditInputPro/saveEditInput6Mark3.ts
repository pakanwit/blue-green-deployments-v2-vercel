...
export default async function handler(request, response) {
  const saveEditInput6Mark3Handler = async (req, res) => {
  ...
  const saveEditInput6Mark3HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput6Mark3Handler,
  );

  await saveEditInput6Mark3HandlerWithApiKeyValidation(request, response);
}
