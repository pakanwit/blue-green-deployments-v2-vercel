...
export default async function handler(request, response) {
  const saveEditInput12Tech1Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputOp:', editInputTech1);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputTech1s.push(
            editInputTech1,
          );
        } else {
  ...
  const saveEditInput12Tech1HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput12Tech1Handler,
  );

  await saveEditInput12Tech1HandlerWithApiKeyValidation(request, response);
}
