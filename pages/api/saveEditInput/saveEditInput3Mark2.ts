...
export default async function handler(request, response) {
  const saveEditInput3Mark2Handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      console.log('editInputMark:', editInputMark2);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMark2s.push(
            editInputMark2,
          );
        } else {
  ...
  const saveEditInput3Mark2HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput3Mark2Handler,
  );

  await saveEditInput3Mark2HandlerWithApiKeyValidation(request, response);
}
