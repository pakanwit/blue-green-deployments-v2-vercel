...
export default async function handler(request, response) {
  const saveEditInput2situHandler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputSitu:", editInputSitu);
  ...
  const saveEditInput2situHandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput2situHandler
  );

  await saveEditInput2situHandlerWithApiKeyValidation(request, response);
}
