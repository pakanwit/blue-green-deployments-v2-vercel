...
export default async function handler(request, response) {
  const saveEditInput15Mang2Handler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputMang:", editInputMang2);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMang2s.push(
            editInputMang2
          );
        } else {
  ...
  const saveEditInput15Mang2HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput15Mang2Handler
  );

  await saveEditInput15Mang2HandlerWithApiKeyValidation(request, response);
}
