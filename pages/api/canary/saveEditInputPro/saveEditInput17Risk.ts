...
export default async function handler(request, response) {
  const saveEditInput17RiskHandler = async (req, res) => {
    console.log("saveEditInputRisk running");
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputRisk:", editInputRisk);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          // Check if editInput object and editInputRisk array exists, if not, initialize it
          ...
          // Now you can safely push the new editInputRisk into the array
          user.plans[planIdNum].editedVer.editInput.editInputRisks.push(
            editInputRisk
          );
        } else {
  ...
  const saveEditInput17RiskHandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput17RiskHandler
  );

  await saveEditInput17RiskHandlerWithApiKeyValidation(request, response);
}
