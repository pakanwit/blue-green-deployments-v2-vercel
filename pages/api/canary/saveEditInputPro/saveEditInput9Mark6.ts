...
export default async function handler(request, response) {
  const saveEditInput9Mark6Handler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputMark:", editInputMark6);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMark6s.push(
            editInputMark6
          );
        } else {
  ...
  const saveEditInput9Mark6HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput9Mark6Handler
  );

  await saveEditInput9Mark6HandlerWithApiKeyValidation(request, response);
}
