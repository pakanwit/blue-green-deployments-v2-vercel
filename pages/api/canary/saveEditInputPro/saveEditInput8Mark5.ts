...
export default async function handler(request, response) {
  const saveEditInput8Mark5Handler = async (req, res) => {
    ...
    if (req.method === "POST") {
      ...
      console.log("editInputMark:", editInputMark5);
      ...
      try {
        ...
        if (user.plans && user.plans.length > 0) {
          ...
          user.plans[planIdNum].editedVer.editInput.editInputMark5s.push(
            editInputMark5
          );
        } else {
  ...
  const saveEditInput8Mark5HandlerWithApiKeyValidation = withApiKeyValidation(
    saveEditInput8Mark5Handler
  );

  await saveEditInput8Mark5HandlerWithApiKeyValidation(request, response);
}
