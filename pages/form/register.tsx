...
export default function Register1({ secretKey, fbPixelId }) {
  ...
  const fetchReviews = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/trustpilot/reviewsRegister`,
      {
        headers: {
  ...
  async function onSubmit(
    ...
    await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/signup`, options)
      .then(async (res) => {
        if (res.status === 422) {
          const data = await res.json();
    ...
    if (
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/sendEmailToZapier`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
